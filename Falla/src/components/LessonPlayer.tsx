  ...
  return (
    <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
      ...
      >
        ...
        <div 
          className="p-5 border-b-2 flex items-center justify-between gap-4"
          style={{
            borderBottomColor: 'var(--theme-border)',
            backgroundColor: 'var(--theme-bg)'
          }}
        >
          <button onClick={() => { speechService.stop(); onCancel(); }} className="text-slate-400 hover:text-slate-600 transition-all font-black text-xs uppercase tracking-wider cursor-pointer">
            ✕ Fechar
          </button>
        ...
        <div className="p-6 md:p-8 space-y-6">
          ...
          <div className="flex gap-4 items-start">
            ...
            <div className="relative bg-slate-50 border-2 border-slate-200 rounded-3xl p-4.5 text-xs font-bold text-slate-700 flex-1 leading-relaxed shadow-2xs">
              ...
              {question.text && (
                <div className="mt-2 text-falla-blue font-black flex items-center gap-2 bg-white p-2 px-3 rounded-xl border-2 border-slate-200 w-fit shadow-2xs">
                  <AudioButton text={question.text} compact onError={setAudioError} />
                  <span className="text-xs tracking-wide">{question.text}</span>
                </div>
              )}
              {audioError && (
                <p className="mt-2 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-[10px] font-bold text-red-700">
                  {audioError}
                </p>
              )}
            </div>
          ...
          <div className="py-2">
                ...
                {question.options?.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        borderColor: isSel ? 'var(--theme-primary)' : undefined,
                        backgroundColor: isSel ? 'var(--theme-primary-light)' : undefined,
                        color: isSel ? 'var(--theme-primary-dark)' : undefined
                      }}
                    >
                      <span className="flex min-w-0 items-center gap-2">
                        <AudioButton text={opt} compact onError={setAudioError} />
                        <span className="break-words">{opt}</span>
                      </span>
                      <div 
                        className="w-4.5 h-4.5 rounded-full border-2 flex items-center justify-center shrink-0"
                        style={{
                          borderColor: isSel ? 'var(--theme-primary)' : '#cbd5e1',
                          backgroundColor: isSel ? 'var(--theme-primary)' : 'transparent'
                        }}
                      >
                  ...
                  {selectedWords.map((word) => (
                    ...
                    >
                      <span className="flex items-center gap-2">
                        <AudioButton text={word} compact onError={setAudioError} />
                        {word}
                      </span>
                    </button>
                  ))}
            ...
            {question.type === QuestionType.SPEAK_SIM && (
              <div className="flex flex-col items-center justify-center py-6 space-y-4">
                <AudioButton
                  text={question.text || (Array.isArray(question.correctAnswer) ? question.correctAnswer.join(' ') : question.correctAnswer)}
                  label="Ouvir antes de falar"
                ...
                <button
                  ...
                  className={`w-20 h-20 rounded-full flex items-center justify-center shadow-lg border-4 transition-all text-white active:scale-95`}
                  style={{
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2 max-w-sm mx-auto">
                  {question.options?.map((opt, idx) => (
                    <div key={idx} className="flex items-center justify-center gap-2 p-3 bg-slate-50 border border-slate-200 rounded-xl text-center font-bold text-xs text-slate-700">
                      <AudioButton text={opt} compact onError={setAudioError} />
                      <span>{opt}</span>
                    </div>
                  ))}
            ...
            {!hasChecked ? (
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  ...
                  style={{
                    backgroundColor: ((question.type === QuestionType.MULTIPLE_CHOICE && !selectedOpt) || (question.type === QuestionType.SENTENCE_BUILDER && selectedWords.length === 0))
                      ? undefined
                      : 'var(--theme-primary)',
                    borderBottomColor: ((question.type === QuestionType.MULTIPLE_CHOICE && !selectedOpt) || (question.type === QuestionType.SENTENCE_BUILDER && selectedWords.length === 0))
                      ? undefined
                      : 'var(--theme-primary-dark)'
                  }}
                >
              ...
              <div className={`p-4.5 rounded-3xl border-2 flex flex-col md:flex-row items-center justify-between gap-4 animate-fade-in ${
                ...
                <div className="flex gap-2.5 w-full md:w-auto shrink-0 mt-3 md:mt-0">
                  <button
                    onClick={askGeminiTutor}
                    className="text-white font-black text-xs px-4 py-2.5 rounded-2xl flex items-center gap-1 transition-all flex-1 md:flex-initial active:translate-y-0.5 active:border-b-0 cursor-pointer shadow-2xs shrink-0"
                    style={{
                      backgroundColor: 'var(--theme-accent)',
                      borderBottom: '4px solid var(--theme-primary-dark)'
                    }}
                  >
                    <Lightbulb size={14} />
                    Tutor Explica
                  ...
                  <button
                    ...
                    className="font-black text-xs py-2.5 px-5 rounded-2xl border-b-4 shadow-sm flex items-center justify-center gap-1 transition-all flex-1 md:flex-initial active:translate-y-0.5 active:border-b-0 cursor-pointer text-white"
                    style={{
                      backgroundColor: isCorrect ? 'var(--theme-success, #34d399)' : 'var(--theme-primary)',
                      borderBottomColor: isCorrect ? '#22c55e' : 'var(--theme-primary-dark)'
                    }}
                  >
                    Avançar
                    <ArrowRight size={14} />
      ...
      <AnimatePresence>
        {showAiPanel && (
          <motion.div 
            ...
            exit={{ opacity: 0, x: 20 }}
            className="rounded-3xl border-2 shadow-lg p-5 space-y-4"
            style={{
              backgroundColor: 'var(--theme-card-bg)',
              borderColor: 'var(--theme-border)'
            }}
          >
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <span 
                className="font-black text-xs flex items-center gap-1 uppercase tracking-wide"
            ...
            {aiLoading ? (
              <div className="py-12 flex flex-col items-center justify-center space-y-3">
                <div 
                  className="w-8 h-8 rounded-full border-4 border-slate-100 animate-spin" 
